...
# Scale (radio bubble diameter) in lightyears:
SCALE = 225  # enter 225 to see the Earths radio bubble

# Number of advances civilizations from the drake equation
NUM_CIVS = 15600000
...
def detect_prob(disc_vol_scaled):
    """Calculate probability of galactic cilivization deteecting each other"""
    ...
    if ratio < 0.002:
        ...
        detection_prob = -0.004757 * ratio**4 + 0.06681 * \
            ratio**3 - 0.3605 * ratio**2 + 0.9215 * ratio + 0.00826
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    for i in range(theta_max_degrees):  # range(0,600,2) for no blackhole
...
def main():
    ...
    star_haze(disc_radius_scaled, density=9)
